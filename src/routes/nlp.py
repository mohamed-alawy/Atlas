...
async def index_project(request: Request, project_id: int, push_request: PushRequest):
    ...
    return JSONResponse(
        content={
            "message": ResponseStatus.INDEXING_STARTED.value, 
            "task_id": task.id
            }
    )
...
async def get_project_index_info(request: Request, project_id: int):
    ...
    collection_info = await nlp_controller.get_vector_db_collections_info(project=project)
...
async def search_index(request: Request, project_id: int, search_request: SearchRequest):
    ...
    if not answer:
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"message": ResponseStatus.RAG_ANSWER_GENERATION_FAILED.value}
        )
    
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"message": ResponseStatus.RAG_ANSWER_GENERATION_SUCCESS.value,
                "answer": answer,
